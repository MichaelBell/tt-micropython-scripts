...
async def do_quad_read(dut, addr, length):
    ...
    for j in range(length):
        ...
        for i in range(2):
            d <<= 4
            d |= dut.spi_q_data_out.value.to_unsigned()
            await cycle_clock(dut)
        data.append(d)
    dut.spi_select.value = 1
    await Timer(10, "ns")
...
async def do_quad_write(dut, addr, data):
    ...
    for i in range(24):
        ...
        addr <<= 1
        await cycle_clock(dut)
    dut.tt.uio_oe_pico.value = 0xf
    for j in range(len(data)):
        d = data[j]
        assert dut.uio_oe.value == 0b11110000
        for i in range(2):
            dut.spi_q_data_in.value = (d & 0xf0) >> 4
            d <<= 4
            await cycle_clock(dut)
    dut.spi_select.value = 1
    dut.tt.uio_oe_pico.value = 1
...
def main():
    ...
    class MaskedEqValue:
        def __init__(self, def_value:int=0, mask:int=0xFF):
            self._value = def_value
            self._mask = mask
        
        def __eq__(self, other):
            return (self._value & self._mask) == (other & self._mask)
    ...
    class DUT(DUTWrapper):
        def __init__(self):
            ...
            # Clock reconfig
            self.debug_clk = self.clk
            
            # inputs
            self.debug_addr = self.new_slice_attribute(self.tt.ui_in, 5, 2)
            self.spi_select = self.new_slice_attribute(self.tt.ui_in, 1)
            self.clk = self.new_slice_attribute(self.tt.ui_in, 0)
            self.spi_mosi = self.new_slice_attribute(self.tt.uio_in, 0)
            self.spi_q_data_in = self.new_slice_attribute(self.tt.uio_in, 3, 0)
            
            # outputs
            self.segments = self.new_slice_attribute(self.tt.uo_out, 6, 0)
            self.dot = self.new_slice_attribute(self.tt.uo_out, 7)
            self.spi_miso = self.new_slice_attribute(self.tt.uio_out, 1)
            self.spi_q_data_out = self.new_slice_attribute(self.tt.uio_out, 3, 0)
            self.debug_data = self.new_slice_attribute(self.tt.uio_out, 7, 4)
            
            # Fake output enable
            self.uio_oe = FakeSignal(MaskedEqValue(0b11110000, 0b11110001))
    
    tt = DemoBoard.get()
    tt.shuttle.tt_um_MichaelBell_spi_peri.enable()
    tt.clock_project_stop()
    tt.uio_oe_pico.value = 1 # MOSI is output
    Clock.clear_all()
